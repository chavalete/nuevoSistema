<div id="modal" class="inicio">
    <div class="modal_title">
        <span class="title">Alta de Cobro</span>
        <!-- Tooltip -->
        <div class='tooltip'>
                    ...
                    <form class="modal">
                        ...
                        <table>
                            <tr>
                                ...
                                <td class="field-input"><input type="text" class="autocomplete" id="remito" required" name="facturasPendientes"/></td>
                            </tr>
                            <tr style="height: 1px;">
        ...
        <div>
            <table style="width: 100%;">
                <tr>
                    ...
                    <td class="cabecera-detalle-blue remitos"></td>
                </tr>
                <tr>
...
<script type="text/javascript">
    ...
    var showSecundary = function(){
        ...
        tr = '<tr id="product_'+product_count+'" class="'+trClass+'">' + tdForma + tdImp  +  tdSupr + '</tr>';
        trResumen = '<tr id="product_'+product_count+'" class="'+trClassResumen+'">' + tdForma + tdImp +  '</tr>';
    ...
    $('div.send-button.resumenAltaStock').click(function(){
        if(window.added < 1){
            ...
            if($('input#error', 'div#modal.inicio').val() == '0'){
                ...
                $('td.cabecera-detalle-blue.remitos').html($('input[name=facturasPendientes]').val());
    ...
    var cargarImporte = function(datos){
        var form = $('form.modal', 'div#modal');
        $('input[name=importe]', form).val(typeof(datos.importe) == 'undefined' ? '' : datos.importe);
    }
    var vaciarImporte = function(){
        var form = $('form.modal', 'div#modal');
        $('input[name=importe]', form).val('');
    }
    
    
    // Ajax para cargar la direccion del cliente dinamicamente
    $("#remito").blur(function() {
        if($(this).val() != 1){
            {
                $.post(
                    'inc/process.php',
                    {   
                        accion  : 'traerDatos',
                        id      : $('input[name=facturasPendientes]').attr('params'),
                        tipo    : 'facturas'
                    }, 
                    function(data){
                        if(data.soyError == false){
                            ...
                            $.msgBox({ title: "Error", content: 'No se pudo cargar el importe'});
                        }
                    },
                    'json'
                );   
    ...
    $('div.hacerAltaStock').click(function(){
        ...
        var params = {
            ...
            remitoId       : $('input[name=facturasPendientes]').attr('params'),
