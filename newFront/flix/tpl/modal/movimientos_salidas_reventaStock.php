<div id="modal" class="inicio">
    <div class="modal_title">
        <span class="title">Reventa de stock</span>
        <!-- Tooltip -->
        <div class='tooltip'>
            <div class="tooltip-position" style='right: 30px; float: right;'>
                <img class="tooltip-click" src="img/ayuda.png" />
                <div class="tooltip-content" params="reventa" style="top: 30px; left: -300px; width: 320px; height: 365px;"></div>
            </div>
        </div>
        <!-- Fin de la Tooltip -->
    </div>
                                ...
                                <td colspan="3">
                                    <table class="parcial">
                                        ...
                                        <tr>
                                            ...
                                            <td class="field-input"><input type="text" name="productoCosto" class="parcial required"/></td>
                                            <td class="addButton"></td>
                                        </tr>
                                        <tr>
                                            <td class="field-name"><span class="field-name">Precio</span></td>
                                            <td class="field-input"><input type="text" name="productoPventa" class="parcial required"/></td>
                                            <td class="addButton"></td>
                                        </tr>            
                                    </table>
                                <td>
...
<script type="text/javascript">
    ...
    var showSecundary = function(){
        ...
        var tdCosto = '<td params="'+$('input[name=productoCosto]').val()+'">'+$('input[name=productoCosto]').val()+'</td>';
        var tdPrecio = '<td params="'+$('input[name=productoPventa]').val()+'">'+$('input[name=productoPventa]').val()+'</td>';
        ...
        tr = '<tr id="product_'+product_count+'" class="'+trClass+'">'  + tdProd + tdCosto + tdPrecio + tdCant + tdSupr + '</tr>';
        trResumen = '<tr id="product_'+product_count+'" class="'+trClassResumen+'">' + tdProd + tdCosto + tdPrecio + tdCant + '</tr>';
    ...
    $('input[name=productoPventa]').live('keypress', function(e){
        var keyCode = e.which;    
        if(keyCode == 13){
            hacerAlta();
        }
    ...
    var cargarCosto = function(datos){
        ...
        $('input[name=productoCosto]', form).val(typeof(datos.productoPcosto) == 'undefined' ? '' : datos.productoPcosto);
    }
    var vaciarCosto = function(){
        var form = $('form.modal', 'div#modal');
        $('input[name=productoCosto]', form).val('');
    }
