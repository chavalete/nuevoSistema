                                ...
                                <td colspan="3">
                                    <table class="parcial">
                                        ...
                                        <tr>
                                            ...
                                            <td class="field-input"><input type="text" name="cantidadP"  class="parcial required"/></td>
...
<script type="text/javascript">
    ...
    var showSecundary = function(){
        ...
        if(!$('input[name=clientes]').attr('params')){
            error = true;
            $.msgBox({ 
                title: "Alerta", 
                content: 'Debe ingresar un cliente antes de agregar productos.',
                afterClose: function(){
                    error = false;
                }
            });
        ...
        $('tr', 'table#productos tbody').each(function(){
            if($('input[name=productosCant]').attr('params') == $(this).attr('id_p')){
                existProduct = true;
            }            
        });
        ...
        if(existProduct){
            $.msgBox({ 
                title: "Alerta", 
                content: 'No se puede ingresar dos veces el mismo producto',
                afterClose: function(){
                    error = false;
                }
            });
        ...
        if(id_f != 'undefined'){
            $('tr', 'table#productos tbody').each(function(){
                ...
                if($(this).attr('id_f') == id_f){
                    ...
                    $('td.pt', tr)
                        .attr('value', rs.precio * $('.editableCount').val())
                        .text(rs.precio * $('.editableCount').val());
                }
            });
        }
    ...
    var reCalculatePromo = function(id_f, edit = false, el = null){
        ...
        if(el != null){
            ...
            params.push({
                ...
                idListaPrecio   : $('input[name=clientes]').attr('idlistaprecio')
            });
        }else{
            params.push({
                ID              : $('input[name=productosCant]').attr('params'), 
    ...
    var calculateTotal = function(){
        ...
        var total = 0;
        $('tr', 'table#productos tbody').each(function(){
            total += parseInt($('td.pt', $(this)).attr('value'));
        });
    ...
    $('.editableCount').live('blur', function(e){
        e.stopImmediatePropagation();

        // Si no hay cambios en el input, no hago nada
        ...
        var pt   = $(this).val() * parseInt($('td.pu', el).html());
