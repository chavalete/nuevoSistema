...
require_once '/var/www/html/newFront/afipPropia/soap/FrenteAfip.php';
//ALICUOTA IVA 5=21 4=10.5
...
class facturaElectronica{
    ...
    public function enviarFactura($facturaId){
    
    $qry="SELECT 
            factura_id,
            factura_total_fact AS total_factura,
            factura_total_iva AS total_iva,
            factura_total_fact-factura_total_iva  AS total_neto,
            factura_puesto_venta::integer,
            factura_tipo,
            to_char(factura_fecha,'YYYYMMDD') AS factura_fecha,
            cliente_nombre,
	    ...
	    AND factura_tipo NOT IN (3,8) ORDER BY factura_id;";
    
    //echo $qry;exit;
    $this->_db->setQry($qry);
    $resultado=$this->_db->ejecutar();
    if(count($resultado)==0){
        echo "Sin Facturas a generar";exit;
    }
    $this->FrenteAfip->validarCredenciales();
    //exit;
    foreach($resultado AS $detalle){
	    //echo "entra";exit;
            $arrIVA = $this->calcularAlicuotas($detalle['factura_id']);
            //var_dump($arrIVA);exit;
	    $detalle['factura_nro']=$this->FrenteAfip->traerUltimoComprobante($detalle);
	    echo $detalle['factura_nro'];exit;
       exit;
            $factura = array(
                'FeCAEReq'=>
                array(
                ...
                'FECAEDetRequest'=> array(
                    ...
                    'DocTipo' => $detalle['tipo_doc'],              // 96: DNI, 80: CUIT, 99: Consumidor Final
                    //'DocNro' => 20000000000,    // Nro. de CUIT o DNI
        ...
        foreach ($this->FrenteAfip->getUltimaRespuesta() AS $respuesta){
            ...
            //$this->_db->addCamposUpdate('observaciones= \'' . $facturas[0]['motivos_obs'] .'\'');
                    ...
                    $this->_db->addCamposUpdate('pedido_factura_nro = \'' . $facturaNro .'\'');
                    $this->_db->addFrom('datos_pedidos');
                    $this->_db->addWhere('pedido_id = \'' . $detalle['pedido_id'] .'\'');
                    $this->_db->generarUpdate();
                    $qry.=$this->_db->getQry();
