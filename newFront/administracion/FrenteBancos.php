...
class FrenteBancos
{
   ...
   public function setArrError ($error){
    ...
    public function buscarBancos ($arrParametros){
	
        //**ANALIZO EL WHERE**//
        if($arrParametros['stringBuscar']){
            $this->_db->addWhere('banco_nombre ILIKE  \'%' . $arrParametros[stringBuscar] .'%\'');
            $primerRegistro = 0;
            $ultimoRegistro = 11;
	    }else{
        ...
        for($i=$primerRegistro;$i<$ultimoRegistro;$i++){   
            $array['bancoId'] = $arr[$i]['banco_id'];
            $array['bancoNombre'] = $arr[$i]['banco_nombre'];
            $this->_colBancos[]= $array;
        }
    ...
    public function setOrdenBancos($parametros){
        ...
        {
            //definir default
            $this->_ordenColumnas['ordenarPor']    =   'banco_nombre';
    ...
    public function getDetalles(){
        ...
        foreach($this->_colBancos AS $objConsulta){
                $arr[] = array(
    ...
    public function getDetallesAutocompletar(){
        ...
        foreach($this->_colBancos AS $banco){
            $arr[]   =   array(
                    'label' =>  $banco['bancoNombre'],
                    'value' => $banco['bancoId']
                    );
        }		
	return $arr;
