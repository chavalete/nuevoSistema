...
class MailAlertaStockMinimo{
    ...
    public function enviar(){
        ...
        $this->_db_dM->addWhere('sc.cantidad <=dp.stock_minimo');
        ...
        if(count($rsVencimiento) > 0 ){
            ...
            $body= "<font  color=red ><h3> ALERTA!  stock de productos. </h3></font>";
            $body.= "<table border=2><tr><th>Cant.</th><th> Stock Minimo</th><th> Producto</th><tr>";
            
            foreach ($rsVencimiento as $productos){
                $body.="<tr><td>" . $productos['cantidad'] . "</td>";
                $body.="<td>" . $productos['stock_minimo'] . "</td>";
                $body.="<td>" . utf8_decode($productos['producto_nombre']) . "  " . utf8_decode($productos['producto_nombre']) ."</tr>";
            ...
            $vencimiento['fromname'] = "Limon Reparto Stock Minimo"; 
            $vencimiento['addreplyto'][0]= array('address'=> 'alertaslimonsj@gmail.com', 'name'=>'LIMON Envios');
            //destinatarios
	    ...
	    //$vencimiento['addaddress'][] = array('address' => 'mariana@shospitalarias.com.ar', 'name' => 'Mariana');	    
            ...
            $vencimiento['subject'] = " Stock Minimo -LIMON REPARTO";
...
$mail = new MailAlertaStockMinimo();
$mail->enviar();
