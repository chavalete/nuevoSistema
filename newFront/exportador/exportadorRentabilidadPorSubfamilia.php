...
if($_GET){
	    ...
	    $db->addSelect("sum(importe_detalle) as total_detalle,sum(de.producto_pcosto * cantidad) AS total_costo, subfamilia_nombre, sum(cantidad) AS cantidad_vendida, sum(importe_detalle-(de.producto_pcosto*cantidad)) AS ganancia_neto,  sum(de.producto_comision*cantidad) AS total_comision");
	    ...
	    $db->addGroup("subfamilia_nombre");
	    $db->addOrderBy("subfamilia_nombre");
	    $db->generarSelect();
	    ...
	    fwrite($f,$linea);
	    $linea="Subfamilia;Cantidad;Costo Total;Importe Total;Rentabilidad Absoluta;Rentabilidad Relativa;Costo Total comision;Rentabilidad Absoluta comision ;Rentabilidad Relativa comision;\n";
	    fwrite($f,$linea);
	    $totalGanancia=0;
	    ...
	    foreach($resultado AS $productos){
            ...
            $linea = $productos['subfamilia_nombre'].$sep.$productos['cantidad_vendida'].$sep.$objFuncionesComunes->formatoMoneda($costoTotal).$sep.$objFuncionesComunes->formatoMoneda($productos['total_detalle']).$sep.$objFuncionesComunes->formatoMoneda($utilidadAbs).$sep.$utilidadRel.$sep.$objFuncionesComunes->formatoMoneda($costoTotalComision).$sep.$objFuncionesComunes->formatoMoneda($utilidadAbsComision).$sep.$utilidadRelComision ."\n";
