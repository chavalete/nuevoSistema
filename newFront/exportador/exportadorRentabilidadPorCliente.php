...
if($_GET){
	    ...
	    $db->addSelect("sum(importe_detalle) as total_detalle,sum(de.producto_pcosto * cantidad) AS total_costo, cliente_nombre, sum(cantidad) AS cantidad_vendida, sum(importe_detalle-(de.producto_pcosto*cantidad)) AS ganancia_neto,  sum(de.producto_comision*cantidad) AS total_comision");
	...
	$db->addOrderBy("sum(importe_detalle-(de.producto_pcosto*cantidad)) DESC");
	    ...
	    fwrite($f,$linea);
	    $linea="Cliente;Costo Total;Importe Total;Rentabilidad Absoluta;Rentabilidad Relativa;Costo Total comision;Rentabilidad Absoluta comision ;Rentabilidad Relativa comision;\n";
	    fwrite($f,$linea);
	    $totalGanancia=0;
	    ...
	    foreach($resultado AS $productos){
            ...
            $linea = $productos['cliente_nombre'].$sep.$costoTotal.$sep.$productos['total_detalle'].$sep.$utilidadAbs.$sep.$utilidadRel.$sep.$costoTotalComision.$sep.$utilidadAbsComision.$sep.$utilidadRelComision ."\n";
            $totalGanancia= $totalGanancia + $utilidadAbs;
            $totalGananciaComision= $totalGananciaComision + $utilidadAbsComision;
            $linea = str_replace(".",",",$linea);
            fwrite($f,$linea);
	    }
	    $linea= $sep.$sep.$sep.$totalGanancia.$sep.$sep.$sep.$totalGananciaComision."\n";
	    $linea = str_replace(".",",",$linea);
		fwrite($f,$linea);
		fclose($f); 
