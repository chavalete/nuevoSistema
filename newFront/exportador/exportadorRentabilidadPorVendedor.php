...
if($_GET){
	    ...
	    $db->addSelect("sum(importe_detalle) as total_detalle,sum(de.producto_pcosto * cantidad) AS total_costo, vendedor_nombre, sum(cantidad) AS cantidad_vendida, sum(importe_detalle-(de.producto_pcosto*cantidad)) AS ganancia_neto,  sum(de.producto_comision*cantidad) AS total_comision");
        $db->addFrom('datos_facturas');
        $db->addFrom('INNER JOIN detalle_facturas  de USING(factura_id)');
        $db->addFrom('INNER JOIN datos_vendedores USING(vendedor_id)');
	    ...
	    $db->addGroup("vendedor_nombre");
	    $db->addOrderBy("vendedor_nombre");
	    $db->generarSelect();
	    ...
	    fwrite($f,$linea);
	    $linea="Vendedor;Costo Total;Importe Total;Rentabilidad Absoluta;Rentabilidad Relativa;Costo Total comision;Rentabilidad Absoluta comision ;Rentabilidad Relativa comision;\n";
	    fwrite($f,$linea);
	    $totalGanancia=0;
	    ...
	    foreach($resultado AS $productos){
            ...
            $linea = $productos['vendedor_nombre'].$sep.$objFuncionesComunes->formatoMoneda($costoTotal).$sep.$objFuncionesComunes->formatoMoneda($productos['total_detalle']).$sep.$objFuncionesComunes->formatoMoneda($utilidadAbs).$sep.$utilidadRel.$sep.$objFuncionesComunes->formatoMoneda($costoTotalComision).$sep.$objFuncionesComunes->formatoMoneda($utilidadAbsComision).$sep.$utilidadRelComision ."\n";
            $totalGanancia= $totalGanancia + $utilidadAbs;
            $totalGananciaComision= $totalGananciaComision + $utilidadAbsComision;
            $linea = str_replace(".",",",$linea);
            fwrite($f,$linea);
	    }
	    $linea= $sep.$sep.$sep.$objFuncionesComunes->formatoMoneda($totalGanancia).$sep.$sep.$sep.$objFuncionesComunes->formatoMoneda($totalGananciaComision)."\n";
	    $linea = str_replace(".",",",$linea);
		fwrite($f,$linea);
		fclose($f); 
