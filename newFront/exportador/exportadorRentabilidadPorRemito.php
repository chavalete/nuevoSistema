...
if($_GET){
	    ...
	    $db->addSelect("sum(importe_detalle) as total_detalle,sum(de.producto_pcosto * cantidad) AS total_costo, factura_nro, sum(cantidad) AS cantidad_vendida, sum(importe_detalle-(de.producto_pcosto*cantidad)) AS ganancia_neto,  sum(de.producto_comision*cantidad) AS total_comision");
	    ...
	    fwrite($f,$linea);
	    $linea="Nro. Remito;Costo Total;Importe Total;Rentabilidad Absoluta;Rentabilidad Relativa;Costo Total comision;Rentabilidad Absoluta comision ;Rentabilidad Relativa comision;\n";
	    fwrite($f,$linea);
	    $totalGanancia=0;
	    ...
	    foreach($resultado AS $productos){
            ...
            $linea = $productos['factura_nro'].$sep.$objFuncionesComunes->formatoMoneda($costoTotal).$sep.$objFuncionesComunes->formatoMoneda($productos['total_detalle']).$sep.$objFuncionesComunes->formatoMoneda($utilidadAbs).$sep.$utilidadRel.$sep.$objFuncionesComunes->formatoMoneda($costoTotalComision).$sep.$objFuncionesComunes->formatoMoneda($utilidadAbsComision).$sep.$utilidadRelComision ."\n";
