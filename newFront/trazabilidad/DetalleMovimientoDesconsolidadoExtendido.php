 ...
 * @author root
 */
class DetalleMovimientoDesconsolidadoExtendido extends DetalleMovimiento
{
    private $_db;
    ...
    public function salvarMe($arrParametros){
        $qryDetalle = '';
	if($arrParametros['tipo_movimiento_id'] == 11){
            ...
            foreach($arrParametros['codigos'] as $codigo){ 
		      ...
		      "soyError" => true,
		      "nivel" => 1,
		      "mensaje" =>'El codigoaaaa : ' . $codigo . ' ya fue dado de baja con anterioridad, verifique los movimientos del mismo.'
		    );
                    echo json_encode($arrDevolver);exit;
                }
        ...
        }elseif($arrParametros['tipo_movimiento_id'] == 12){
		...
		$this->setIdMovimiento($arrParametros['id_movimiento']);
                ...
                foreach($arrParametros['codigos'] as $trazaCodigo){
                    ...
                    $objTraza->cargarPorCodigo($trazaCodigo);
                    ...
                    if($objTraza->getEsAgrupador()){
                        ...
                        $resultado = $this->_db->ejecutar();
                        $cantidadAgrupada+=count($resultado);
                        foreach ($resultado as $trazasALiberar){
                            $objTrazaALiberar=new DatoTrazabilidadExtendido();
                            $objTrazaALiberar->cargarme($trazasALiberar['trazabilidad_id']);
                            $this->setColObjDatoTraza($objTrazaALiberar);
                            $arrTrazasIds[]=$trazasALiberar['trazabilidad_id'];
                        }
                    }  else {
                        $arrDevolver = array(  "soyError" => true,
                                                "nivel" => 1,
                                                "mensaje" =>'El codigo ingresado no es agrupador : ' . $objTraza->getTrazabilidadCodigo() 
                                            );
                        echo json_encode($arrDevolver);exit;
                    }
                ...
                $txtTrazasIds=  implode(',', $arrTrazasIds);
                ...
                $this->_db->addWhere("trazabilidad_id IN($txtTrazasIds)  AND cancelado = FALSE  ");
		...
		foreach($resultado AS $datos){
			...
			$this->_db->addCamposValue($datos['cantidad']);
                        $this->_db->addCamposValue($datos['unidades'] * $cantidadAgrupada);
			$this->_db->addFrom('detalle_movimientos');
			$this->_db->generarInsert();
			//echo $this->_db->getQry();exit;
			
                        $qryDetalle.= $this->_db->getQry();
                        $qryDetalle.= ' SELECT fn_actualizar_stock_completo('. $datos['unidades'] * $cantidadAgrupada . ',' . $arrParametros['sucursal_id'] . ',' . $arrParametros['almacen_id'] . ',' . $arrParametros['estanteria_id'] . ',' . $datos['lote_id'] . ',' . $datos['producto_id'] . '); ';
                ...
                $this->_db->addCamposUpdate('en_stock = TRUE');
                $this->_db->addFrom('datos_trazabilidad');
                $this->_db->addWhere("trazabilidad_id IN($txtTrazasIds)");
