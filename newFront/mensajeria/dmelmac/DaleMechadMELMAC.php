<?php
require_once $_SERVER['DOCUMENT_ROOT'] . '/newFront/liberia/almacenamiento/FrenteAlmacenamiento.php';
...
class DaleMechadMELMAC
{
    ...
    public function buscarMovimientosReproceso(){
        ...
        $this->_db_dM->addSelect('to_char(f_evento,\'DD/MM/YYYY\') AS f_evento');
        ...
        $this->_db_dM->addSelect('to_char(vencimiento,\'DD/MM/YYYY\') AS vencimiento');
	...
	$this->_db_dM->addFrom('INNER JOIN datos_productos ON mensajeria_anmat.gtin = producto_gtin');
	...
	$idEventoAnterior = 0;
		...
		$objTransaccion = NEW DatoTransaccionExtendido();
                ...
                $objTransaccion->setIdEvento($datoMensajeria['id_evento']);
                $objTransaccion->setFechaEvento($datoMensajeria['f_evento']);
                $objTransaccion->setHoraEvento($datoMensajeria['h_evento']);
		$objTransaccion->setObjGlnOrigen(new GlnOrigenExtendido());
		$objTransaccion->getObjGlnOrigen()->cargarme($datoMensajeria['gln_origen']);
		$objTransaccion->setCuitOrigen($datoMensajeria['cuit_origen'])
		$objTransaccion->setObjGlnDestino(new GlnDestinoExtendido());
		$objTransaccion->getObjGlnDestino()->cargarme($datoMensajeria['gln_destino']);
		...
		$objTransaccion->getObjTipoMovimiento()->cargarMe($arrTransaccion[0]['id_evento']);
		$objTransaccion->setObjErrorAnmat(new ErrorAnmatExtendido());
		$objTransaccion->getObjErrorAnmat()->cargarme($arrTransaccion[0]['error_id']);
                
		if($datoMensajeria['n_factura'] != NULL && $datoMensajeria['n_factura'] != '0'){
		    $this->setNroFactura($datoMensajeria['n_factura']);
		}
		if($datoMensajeria['n_remito'] != NULL && $datoMensajeria['n_remito'] != '0'){
		    $this->setNroRemito($datoMensajeria['n_remito']);
		}
		
                $objTransaccion->setApellido($datoMensajeria['apellido']);
                $objTransaccion->setNombre($datoMensajeria['nombres']);
                $objTransaccion->setNroDocumento($datoMensajeria['n_documento']);
                $objTransaccion->setSexto($datoMensajeria['sexo']);
		...
		$objTransaccion->setNroAsociado($datoMensajeria['nro_asociado']);
                ...
                $objTransaccion->setIdMovimientoSza($datoMensajeria['id_movimiento_sc']);
                
                $objTransaccion->setDisponeNombre($datoMensajeria['dispone_nombre']);
		$objTransaccion->setReferenciaNro($datoMensajeria['referencia_nro']);
        ...
        //fin
        //new de transacciones para enviar!!
