...
class DetalleAutogestionExtendido extends DetalleTransaccion
{
    ...
    public function cargarLazzy($arr){
    ...
    public function setDetalleLazzy($arr){
    
	$this->setDetalleTransaccionId($arr[transaccion_detalle_id]);
	$this->setTrazabilidadCodigo($arr[trazabilidad_codigo]);
	$this->setLote($arr[lote_nro]);
	$this->setLoteVencimiento($arr[lote_vencimiento]);
	$this->setObjProducto(new ProductoExtendidoAnmat());
        $this->getObjProducto()->cargarme($arr['producto_gtin']);
	$this->setProductoGtin($arr['producto_gtin']);
    }

    public function salvarme($objDetalleTransaccion){
    
	$this->_db->addCamposTabla('seq_tmp_detalle_transacciones_transaccion_detalle_id');
	...
	$this->_db->addFrom('detalle_autogestion');
	$this->_db->addWhere('transaccion_id = \'' . $arrParametros['id'] .'\'');
