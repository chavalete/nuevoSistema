...
$db->addSelect('*');
$db->addSelect('to_char(fecha_cobro,\'DD/MM/YY\') AS fecha_cobro');
$db->addSelect('to_char(fecha_cheque,\'DD/MM/YY\') AS fecha_cheque');
...
$db->addFrom('LEFT JOIN datos_localidades ON (datos_clientes.cliente_loca_id=localidad_id)');                             


if($_GET['crIdCobro']!=null){
...
$pdf->Image('limon.jpeg', 115, 10, 65, 25, 'jpg');
...
$pdf->Text(23,17, "Fecha: " .utf8_decode($arrPapa[0]['fecha_cobro'])) ;
//orden
$pdf->SetFont('Times','',12);
$pdf->Text(23,22, "Orden Nro: " .utf8_decode($arrPapa[0]['orden_cobro_nro'])) ;
$pdf->SetFont('Times','',12);
$pdf->Text(23,27, utf8_decode("Autoservicio Mayorista Limón")) ;
...
$pdf->Text(23,32, "Juan M. de Rosas 2667  - San Justo - Bs As.") ;		
$pdf->SetFont('Times','',12);
...
//DIRECCION
$pdf->SetFont('Times','',12);
$pdf->Text(23,45, utf8_decode($arrPapa[0]['direccion'] ." - " . $arrPapa[0]['localidad_nombre']) ) ;
...
$pdf->Text(80,55, utf8_decode("Composición del pago")) ;

$pdf->SetFont('Times','',10);
$pdf->Text(35,60, "Forma Pago");
$pdf->Text(65,60, "Importe");
$pdf->Text(90,60, "Comprobante");
$pdf->Text(120,60, "Fecha Cheque");
$pdf->Text(150,60, "Banco");

$alto = 65;
$importeTotal=0;
foreach ($arrPapa as $detalle){
    
    $pdf->Text(35,$alto, utf8_decode($detalle['forma_desc'])) ;
    $pdf->Text(62,$alto, $objFuncionesComunes->formatoMoneda($detalle['importe_pago']));
    $pdf->Text(93,$alto, utf8_decode($detalle['comprobante_nro']));
    $pdf->Text(123  ,$alto, utf8_decode($detalle['fecha_cheque'])) ;
    $pdf->Text(150,$alto, utf8_decode($detalle['banco_nombre'])) ;
...
//total
$alto = $alto + 4;
$pdf->Text(49,$alto, "TOTAL:") ;
$pdf->Text(62,$alto, $objFuncionesComunes->formatoMoneda($importeTotal));
...
foreach($resultadoFacturas AS $facturas){
    ...
    $alto = $alto + 3;
    if($facturas['guia_id']!=NULL){
        $arrGuias[]=$facturas['guia_id'];
    }
}
$pdf->Line(20, 12 ,20, $alto);
//derecha
