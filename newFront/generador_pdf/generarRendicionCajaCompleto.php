...
class rendicioCaja{
    ...
    public function armarRendicion(){
        $arrCabecera=$this->traerInforCobros();
    }
    public function traerInforCobros(){
        $this->_db->addSelect("to_char(fecha_cobro,'DD-MM-YYYY') AS fecha_cobro");
        $this->_db->addSelect("sum(importe_pago) AS importe_total");
        $this->_db->addSelect("forma_desc");
        $this->_db->addSelect("forma_pago_id");
        ...
        $this->_db->addFrom('INNER JOIN datos_usuarios ON datos_cobros.cobro_usuario_id = datos_usuarios.usuario_id');
        $this->_db->addGroup("fecha_cobro,usuario_nombre_completo, forma_desc, cobro_cancelado, factura_nro, forma_pago_id");
        $this->_db->addOrderby('datos_cobros.fecha_cobro, forma_desc');

        $fecha= $objFuncionesComunes->fechaFormatoDb($_GET['fechaDesde']);;
        if($_GET['fechaDesde']!=null){
            $this->_db->addWhere('fecha_cobro= \'' . $fecha. '\'');
            $this->_db->addWhere('cobro_usuario_id= \'' . $_GET['idUsuario']. '\'');
        }
        $this->_db->addWhere('cobro_cancelado=false');
        $resultado = $this->_db->generarSelect();
        return $resultado;
    }
}
...
//AHORA VAMOS POR HDR
$this->_db = NEW frenteAlmacenamiento();
$this->_db->addSelect("to_char(hoja_estado_fecha,'DD-MM-YYYY') AS fecha_cobro");
$this->_db->addSelect("sum(importe_cobrado) AS importe_total");
$this->_db->addSelect("forma_desc");
$this->_db->addSelect("forma_pago_id");
$this->_db->addSelect("usuario_nombre_completo");
$this->_db->addSelect("factura_nro");
$this->_db->addFrom('datos_hojas_de_ruta dhr');
$this->_db->addFrom('INNER JOIN detalle_hojas_de_ruta dehrd  USING(hoja_id)');
$this->_db->addFrom('INNER JOIN datos_formas_pagos USING(forma_pago_id)');
$this->_db->addFrom('INNER JOIN datos_facturas USING(factura_id)');
$this->_db->addFrom('INNER JOIN datos_usuarios ON dehrd.hoja_estado_usuario_id = datos_usuarios.usuario_id');
$this->_db->addGroup("fecha_cobro,usuario_nombre_completo,hoja_estado_fecha, forma_desc, hoja_anulada, factura_nro, forma_pago_id");
$this->_db->addOrderby('hoja_estado_fecha, forma_desc');

$fecha= $objFuncionesComunes->fechaFormatoDb($_GET['fechaDesde']);;
if($_GET['fechaDesde']!=null){
    $this->_db->addWhere('hoja_estado_fecha::date= \'' . $fecha. '\'');
    $this->_db->addWhere('hoja_estado_usuario_id= \'' . $_GET['idUsuario']. '\'');
}else{
    echo "No llego ID de usuario";exit;
}
$this->_db->addWhere('importe_cobrado>0');
$this->_db->addWhere('hoja_anulada=false');
$this->_db->generarSelect();
//echo $this->_db->getQry();exit;
$arrCabecera =  $this->_db->ejecutar();
