...
class FrentePedidos
{
	...
	foreach($objPedido->getColObjDetallePedido() AS $detalle){
		...
		(
			...
			$detalle->getObjProducto()->getCodigoReferencia(),
	...
	array_pop($arrProductos);
	    ...
	    $arrProductosFinal = explode("#",$linea);
            ...
            $qryPedido['detalle'] .= 'SELECT fn_movimiento_stock(' . $loteAutomatico .' , ' . $arrProductosFinal['idPedido'] . ', ' . $arrProductosFinal['pedidoDetalleId']  . ', ' . $arrProductosFinal['1'] * -1 . ', ' . $arrProductosFinal['0'] . ',' . $loteId . '); ';
	...
	foreach($objPedido->getColObjDetallePedido() AS $detalle){
		...
		(
			...
			$detalle->getObjLote()	->getLote()  . " - " . $this->getDespachoLote($detalle->getObjLote()->getLoteId()) ,
    ...
    public function validarPedido($arrParametros){
        ...
        foreach($arrCodigos AS $codigo){
            $frenteTrazabilidad->cargarPorCodigo($codigo);
        }
        
        $arrCodigosIngresados= array();
        ...
        foreach($frenteTrazabilidad->getColObjDatoTrazabilidad() AS $objTrazabilidad){
            ...
            $arrFinal[ $objProducto->getProductoId()][$objLote->getLoteId()]['ingreso'] += 1  ;
            
        }
        
        //var_dump($arrCodigosIngresados);exit;
        foreach ($this->getColObjDatoPedido() as $objPedido){
            foreach($objPedido->getColObjDetallePedido() AS $objDetallePedido){
                if($arrFinal[$objDetallePedido->getObjProducto()->getProductoId()][$objDetallePedido->getObjLote()->getLoteId()]['ingreso'] == NULL){
                    $arrFinal[$objDetallePedido->getObjProducto()->getProductoId()][$objDetallePedido->getObjLote()->getLoteId()]['ingreso'] = 0;
                }
                $arrFinal[$objDetallePedido->getObjProducto()->getProductoId()][$objDetallePedido->getObjLote()->getLoteId()]['lote'] = $objDetallePedido->getObjLote()->getLote(); 
                $arrFinal[$objDetallePedido->getObjProducto()->getProductoId()][$objDetallePedido->getObjLote()->getLoteId()]['cantidad'] += $objDetallePedido->getPedidoCantidad(); 
                
            }
        }
        
        $txtCodigos="Lote  --  Cantidad  -- Ingreso  <br>";
        $validado = TRUE; 
        foreach($arrFinal as $arrProductos){
            foreach ($arrProductos as $arrLotes){
                    if(abs($arrLotes['cantidad']) != $arrLotes['ingreso']){
                        $validado = FALSE;
                        $color = "red";
                    }else {
                        $color = "blue";
                    }
                    $txtCodigos.= "<font color='{$color}' >" . $arrLotes['lote'] . "   -   " . abs($arrLotes['cantidad']) . "   -   " . $arrLotes['ingreso'] . "</font><br>" ;
            }
        }
        if ($validado == FALSE){
            $arrError['detalle'] = $txtCodigos;
            $arrError['archivo'] = __FILE__;
            $this->_objFrenteError->generarError($arrError);
            $this->_objFrenteError->cargarError();
        }
    ...
    public function getDespachoLote($loteId){
        $this->_db->addSelect('despacho_nro');
        $this->_db->addFrom('datos_movimientos');
        $this->_db->addFrom('INNER JOIN detalle_movimientos USING (id_movimiento)'); 
        $this->_db->addWhere('despacho_nro <> \'\'');
        $this->_db->addWhere('lote_id =' . $loteId); 
        //$this->_db->addGroup('despacho_nro');
        $this->_db->addOrderBy('id_movimiento');
	$this->_db->generarSelect();
        $resultado = $this->_db->ejecutar();
        
        return $resultado[0]['despacho_nro'];
    }
